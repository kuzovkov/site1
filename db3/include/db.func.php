...
/**
* Check existing Db and create it if not exists
* @return  true if Db exists or false if not
**/
function checkDbExisting()
{
    $conn = dbConnect();
    if (!$conn) exit('Error open db'); 
    $databases = dbGetQueryResult('SHOW DATABASES');
    foreach( $databases as $item ) 
        if ( $item['Database'] == DB_NAME ) 
            return true;
    dbCreate(BASE_PATH . DEFAULT_DB_DUMP , true);
