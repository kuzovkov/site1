...
class Discount1_2_3 implements IDiscount {
    ...
    public function __construct(array $productIds, $discountValue, $once = false) {
        $this->products = array();
        $this->discount = 0;
...
class Discount4 implements IDiscount {
    ...
    public function __construct($productId, array $productIds, $discountValue, $once = false) {
        $this->products = array();
        $this->discount = 0;
...
class Discount5_6_7 implements IDiscount {
    ...
    public function __construct($productCount, array $productIdsExcept, $discountValue, $once = false) {
        $this->products = array();
...
class DiscountManager implements IDiscountManager {
    ...
    public function applyDiscounts() {
        ...
        foreach ($this->discounts as $discount) {
            ...
            if ( $discount->getOnce() && in_array( get_class($discount), $this->applied ) ) continue;
			$this->applied[] = get_class($discount);
